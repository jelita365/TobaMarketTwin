...
/**
 * Weighted Sum Model — "Why MCDM?": the configuration space has multiple
 * alternatives evaluated across competing criteria, so a transparent
 * multi-criteria decision rule is used for prototype screening rather than
 * a hidden AI score.
 *
...
export function computeDecisionResults(
    ...
    humanEvaluationsByConfig: Record<string, number> // configId -> respondent count, for calibration gating done by callers
): DecisionResult[] {
    return configs
...
function nextActionFor(result: DecisionResult, evidenceSufficiency: string): DecisionTrace['nextAction'] {
...
function confidenceFor(result: DecisionResult, evidenceSufficiency: string): { status: DecisionTrace['confidenceStatus']; reason: string } {
...
export function buildDecisionTrace(config: ProductConfiguration, result: DecisionResult): DecisionTrace {
    ...
    return {
        ...
        strengths,
        tradeoffs,
        nextAction: nextActionFor(result, ai.evidenceSufficiency),
