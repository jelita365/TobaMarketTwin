...
import { MIN_RESPONSES_FOR_CALIBRATION } from '@/shared/domain';
...
export default function DashboardPage() {
    ...
    const humanResponseCount = allRows.reduce((sum, { config }) => sum + getHumanEvaluations(config.id).length, 0);
    const calibrationEligible = allRows.filter(({ config }) => getHumanEvaluations(config.id).length >= MIN_RESPONSES_FOR_CALIBRATION).length;
